...
val trace_expr_st :
  state ->
  Ast.expression ->
  (Memory.environment * Memory.memory * Ast.expression) list
(* like [trace_expr] but keeps track of environment and memory changes *)

val trace_instr_st :
  state -> conf -> (Memory.environment * Memory.memory * conf) list
(* like [trace_instr] but keeps track of environment and memory changes *)

val trace_st :
  Ast.program -> (Memory.environment * Memory.memory * Ast.expression) list
(* like [trace] but keeps track of environment and memory changes *)
